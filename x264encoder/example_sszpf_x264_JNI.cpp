...
void CALLBACK H264DataCallBackFunc(void* pdata,int datalen,long long time)
{
	LOGI("/**********************H264DataCallBackFunc************************%d",datalen);

	h264datacallback.name = "H264DataCallBackFunc";
	h264datacallback.signature = "([BIJ)V";
	JavaEnv java;
	if (java.istarch) {
		JNIEnv* menv= NULL;
		...
		java.env->CallVoidMethod(ehobj,h264datacallback.getMID(java.env, jclz),pcmdata,datalen,time);
	}
}
